...
void routeHumidityStatus() {
  Serial.println("Route humidity status");
  float h = dht.readHumidity();
  if (isnan(h)) {
     server.send ( 200, "text/plain", "{\"status\": \"unknown\"}");
  }
  else {  
    String response =  "{\"status\": " + String(h) + "}";  
    server.send ( 200, "text/plain", response);
  }
}
...
void setup()
{
  ...
  server.on("/humidity/status", routeHumidityStatus);
